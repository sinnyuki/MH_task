...
setwd("C:/Users/hmz19/Desktop/Mental Health/task/task10/OUTPUT")
dataplot = bruceR::import("C:\\Users\\hmz19\\Desktop\\Mental Health\\task\\task10\\data\\depression_matrix_a_0.xlsx")  #Load data for plot (difference between specific and compound symptoms)
...
a<- ggplot(plot, aes(x=S, y=Scales2, group=S, color=Scales, shape=Type, rev=F)) +
  ...
  geom_hline(yintercept = 1:26, colour = "grey80", size = .2) +
  geom_vline(xintercept = 1:95, colour = "grey80", size = .2) +
  # Points and lines
  ...
  scale_y_continuous(limits=c(0,26.5), expand=c(0,0), breaks=1:26, labels=levels(plot$Scales))+
